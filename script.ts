...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function rechercherEcole() {
        ...
        const ecoleTrouvee = ecoles.find(ecole => ecole["Établissement"] && ecole["Établissement"].toLowerCase().includes(searchInput));
...
<p><strong>Effectif des autres candidats ayant reçu une proposition d’admission:</strong> ${ecoleTrouvee["Effectif des autres candidats ayant reçu une proposition d’admission de la part de l’établissement"]}</p>`
        } else {
    ...
    document.getElementById("searchButton")?.addEventListener("click", rechercherEcole);
});
